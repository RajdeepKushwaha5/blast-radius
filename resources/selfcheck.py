...
def main():
    ...
    # ---- the demo comparison must not go quiet on a directory it cannot open.
    #
    # Raised by a reviewer, and worse than reported. os.walk drops an unreadable
    # directory, and with no callback it says nothing -- so the files inside it leave the
    # BASE tree and come back as files the head tree ADDED. Measured on the bundled
    # fixture: three changed files became five. The silent version does not merely
    # under-report, it invents rows. It is also the path a stranger on a clean machine
    # hits first, which is the worst place to be quiet.
    total += 1
    import shutil as _sh
    import tempfile as _tf
    sys.path.insert(0, HERE)
    import blast as _b
    _d = _tf.mkdtemp(prefix="blast-blocked-")
    try:
        _sh.copytree(os.path.join(HERE, "demo", "head"), os.path.join(_d, "head"))
        _sh.copytree(os.path.join(HERE, "demo", "base"), os.path.join(_d, "base"))
        _H = os.path.join(_d, "head")
        _B = os.path.join(_d, "base")
        _c1, _a1, _u1 = _b.collect_changes_trees(_H, _B)
        _subs = [x for x in sorted(os.listdir(_B))
                 if os.path.isdir(os.path.join(_B, x))]
        if not _subs:
            failures.append({
                "case": "blocked:the-fixture-has-a-directory-to-block",
                "detail": "the bundled base tree has no subdirectory, so this case "
                          "proves nothing on this machine"})
        else:
            _t = os.path.join(_B, _subs[0])
            try:
                os.chmod(_t, 0)
                _c2, _a2, _u2 = _b.collect_changes_trees(_H, _B)
            finally:
                os.chmod(_t, 0o755)
            if not _u2:
                failures.append({
                    "case": "blocked:a-directory-that-will-not-open-is-recorded",
                    "detail": "a directory was made unreadable and the comparison said "
                              "nothing. Its files left the base tree and returned as "
                              "additions: %d changed became %d."
                              % (len(_c1), len(_c2))})
    except Exception as _e:
        failures.append({"case": "blocked:a-directory-that-will-not-open-is-recorded",
                         "detail": "case could not run: %s: %s"
